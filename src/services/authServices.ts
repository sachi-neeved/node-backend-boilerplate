...
/**
 * AuthService provides all authentication-related business logic:
 * registration, OTP verification, login, token creation, session management.
 */
class AuthService extends RepositoryManager {
	/**
	 * Registers a new user with email and password.
	 * Creates the user (with the default "client" role) and issues a fresh OTP.
	 ...
	 * @returns `{ token, hash }` — the raw JWT string and its stored hash.
	 */
	 ...
	 * Called on logout to invalidate the token server-side.
	 *
	 * @param refreshToken - The raw refresh JWT string from the cookie.
	 */
	 ...
	 * @returns The populated user document, or null if not found.
	 */
